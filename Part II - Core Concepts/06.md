...
### Why can a PersistentVolume be used by only one PVC at a time?

### Why do PersistentVolum(Claim) access modes pertain to the number of worker nodes, not to the number of pods?

- 왜 이렇게 했지?

### How to manually reclaim PersistentVolumes (and not delete then restart)
...
### Any PersistentVolume use cases?
