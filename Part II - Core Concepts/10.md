...
> DNS SRV record 를 사용하면 Kubernetes 가 아니더라도 다른 곳에서 사용할 수 있을지 궁금하다. Kubernetes-agnostic 하게 하려고 이렇게 했다고 책에 적혀있는데...
...
> 책에서 얘기하는 *node failure* 의 정의가 정확히 무엇인가? Node failure 면 진짜 node 가 동작을 중단해서 안에 있는 리소스들이 전부 사용이 불가능하게 되어야 하는 것이 아닌지? Node 가 살아있는데, Kubelet 이 노드의 상태를 보고 중단하는 상황도 node failure 로 치는 것인가?
